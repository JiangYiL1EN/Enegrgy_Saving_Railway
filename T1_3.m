...
Ts=[200,210,220,230,250,350,600];
mark=["-","-","--",":","--","--"];
...
for i=1:1:n
   ...
   %制动时间计算
    ...
    mak2=Maks(i);
    %时间/距离
%     plot(S1,(0:1:i1-1)*dt,'r-', 'LineWidth',2);
%     plot([S1(end),S3(1)],[t1,t1+t2],'r-', 'LineWidth',2);
%     plot(S3,(0:1:i3-1)*dt+t1+t2,'r-', 'LineWidth',2)
%     plot(S4,(0:1:i4-1)*dt+(t1+t2+t3),'r-', 'LineWidth', 2);
%     
%     [S1,S1(end),S3(1),S3,S4],[((0:1:i1-1)*dt),t1,t1+t2,((0:1:i3-1)*dt+t1+t2),((0:1:i4-1)*dt+(t1+t2+t3))]
    plot([S1,S1(end),S3(1),S3,S4],[((0:1:i1-1)*dt),t1,t1+t2,((0:1:i3-1)*dt+t1+t2),((0:1:i4-1)*dt+(t1+t2+t3))],'Color',col, 'LineStyle',mak,'Marker',mak2, 'LineWidth',2)
end
